...
/**
 * 017：代入. あるlong型の変数の参照を100万回するだけのスレッドAと、
	...
	public static void main(final String[] args) throws InterruptedException {
		...
		es.submit(() -> {
			for (int i = 0; i < COUNT; i++) {
				System.out.println("スレッドAカウント：" + i);
				assert variable == 0 || variable == -1 : String.format("variable (%d) is not 0 or -1.", variable);
			}
			latch.countDown();
		});
		...
		es.submit(() -> {
			for (int i = 0; i < COUNT; i++) {
				System.out.println("スレッドBカウント：" + i);
